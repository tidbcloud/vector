...
fn arg_to_bool(arg: &ResolvedArgument) -> std::result::Result<bool, Box<dyn DiagnosticError>> {
    arg.expression
        .as_value()
        .as_ref()
...
fn arg_to_case(arg: &ResolvedArgument) -> std::result::Result<Case, Box<dyn DiagnosticError>> {
    if arg_to_bool(arg)? {
        Ok(Case::Sensitive)
    } else {
        Ok(Case::Insensitive)
